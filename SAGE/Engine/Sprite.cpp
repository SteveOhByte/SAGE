...
void Sprite::SetPivot(const Pivot pivot)
{
	...
	if (pTexture != nullptr)
	{
		...
		{
			case UPPER_LEFT:
				origin = Vector2(static_cast<float>(textureRegion.left), static_cast<float>(textureRegion.top));
				break;
			case UPPER_RIGHT:
				origin = Vector2(static_cast<float>(textureRegion.right), static_cast<float>(textureRegion.top));
				break;
			case CENTRE:
				origin = Vector2(static_cast<float>(textureRegion.right - textureRegion.left) / 2.0f, static_cast<float>(textureRegion.bottom - textureRegion.top) / 2.0f);
				break;
			case LOWER_RIGHT:
				origin = Vector2(static_cast<float>(textureRegion.right), static_cast<float>(textureRegion.bottom));
				break;
			case LOWER_LEFT:
				origin = Vector2(static_cast<float>(textureRegion.left), static_cast<float>(textureRegion.bottom));
				break;
		}
	}
}
...
void Sprite::Draw(SpriteBatch* pBatch) const
{
	...
	pBatch->Draw(pTexture->GetResourceView(), position, &textureRegion, colour, rotation, origin, scale, DirectX::SpriteEffects_None, layer);
}
...
bool Sprite::ContainsPoint(Vector2 pos) const
{
	...
	const Vector2 centre = GetCentreNoRotation();
...
Vector2 Sprite::GetCentreNoRotation() const
{
	const float halfWidth = 0.5f * static_cast<float>(GetWidth());
	const float halfHeight = 0.5f * static_cast<float>(GetHeight());
...
void Sprite::SetVelocity(const Vector2 velocityPixPerSec, const float rotationalVelocityDegPerSec)
{
