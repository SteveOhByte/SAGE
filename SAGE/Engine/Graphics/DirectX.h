...
protected:
	...
	struct ScreenSizeBuffer {
		float width;
		float height;
		float padding[2]; // Maintain 16-byte alignment
	};
	
	bool CreateShaders();
	bool CreateVertexBuffer(UINT bufferSize);
	bool UpdateVertexBuffer(void* data, UINT dataSize);
	bool CreateScreenSizeBuffer();
